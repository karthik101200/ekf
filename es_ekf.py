...
def measurement_update(h_jac,sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    i=np.linalg.inv(h_jac@p_cov_check@h_jac.T+sensor_var)
    kal=p_cov_check@h_jac.T@i

    # 3.2 Compute error state
    delx=kal@(y_k-p_check)
    delp=delx[:3]
    delv=delx[3:6]
    delq=delx[6:9]
    ...
    p_hat=p_check+delp
    v_hat=v_check+delv
    q_hat=Quaternion(axis_angle=delq).quat_mult(q_check, out='np')
    

    # 3.4 Compute corrected covariance
    I=np.identity(9)
    p_cov_hat=(I-kal@h_jac)@p_cov_check
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    vf=var_imu_f
    vw=var_imu_w
    Q=(delta_t**2)*np.diag([vf,vf,vf,vw,vw,vw])

    # 1.1 Linearize the motion model and compute Jacobians
    f=imu_f.data[k-1]
    w=imu_w.data[k-1]
    C_ns=Quaternion(C_ns*q_est[k-1]).to_mat()

    p_check=p_est[k-1]+delta_t*v_est[k-1]+(delta_t**2/2)*(C_ns*f+g)
    ...
    F[3:6,6:9]=((-C_ns@f).reshape(3,1))*delta_t
    ...
    if gnss_i<49:
        ...
        q_est[k]=q_check
        p_cov[k]=p_cov_check
        if gavail and liavail:
            y_k=gnss.data[gnss_i]
            p_est[k],v_est[k],q_est[k],p_cov[k]=measurement_update(h_jac,gnss_var, p_cov_check, y_k, p_check, v_check, q_check)
            gnss_i+=1

            y_k=lidar.data[lidar_i]
            p_est[k],v_est[k],q_est[k],p_cov[k]=measurement_update(h_jac,lidar_var, p_cov_check, y_k, p_check, v_check, q_check)
            lidar_i+=1
        if not gavail and liavail:
            y_k=lidar.data[lidar_i]
            p_est[k],v_est[k],q_est[k],p_cov[k]=measurement_update(h_jac,lidar_var, p_cov_check, y_k, p_check, v_check, q_check)
            lidar_i+=1
        if not liavail and gavail:
            y_k=gnss.data[gnss_i]
            p_est[k],v_est[k],q_est[k],p_cov[k]=measurement_update(h_jac,gnss_var, p_cov_check, y_k, p_check, v_check, q_check)
            gnss_i+=1
